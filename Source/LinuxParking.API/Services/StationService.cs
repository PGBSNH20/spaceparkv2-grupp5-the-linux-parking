...
namespace LinuxParking.API.Services
{
    public class StationService : IStationService
    {
        ...
        {
            var existing = await _stationRepository.FindByIdAsync(id).ConfigureAwait(false);
        ...
        {
            var station = await _stationRepository.FindByIdAsync(id).ConfigureAwait(false);
        ...
        public async Task<StationResponse> SaveAsync(Station station)
        {
            try
            {
                await _stationRepository.AddAsync(station).ConfigureAwait(false);
                await _unitOfWork.CompleteAsync().ConfigureAwait(false);
